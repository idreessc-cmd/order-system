...
export const handler = async (event, context) => {
  ...
  try {
    ...
    const { id, name, price, description, category, status, sortOrder } = payload;

    if (!name || !category || !status) {
      return {
        statusCode: 400,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ success: false, message: "بيانات غير مكتملة. الاسم، التصنيف والحالة حقول مطلوبة." })
      };
    }
    ...
    const googleResponse = await fetch(googleScriptUrl, {
      ...
      body: JSON.stringify({
        ...
        subject: { id, name, price, description, category, status, sortOrder }
      }),
    });
    ...
    console.error("Error in admin-save-subject function:", error);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json" },
