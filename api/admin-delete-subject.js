...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!id) {
      return res.status(400).json({ success: false, message: "معرف المادة (id) مطلوب." });
    }
    ...
    console.error("Error in admin-delete-subject API:", error);
    return res.status(500).json({ success: false, message: "حدث خطأ داخلي في /api/admin-delete-subject.", debug: error.message });
  }
}
